...
export function getEntry(path, key) {
  if (!path || !key) return null
  return rowToEntry(getDb(path).prepare('SELECT * FROM ledger WHERE key = ?').get(key))
}
...
export function findRemoteOwnedByTs(path, channel, ts, principal) {
  if (!path || !channel || !ts || !principal) return null
  const entry = findByTs(path, channel, ts)
  if (!entry?.result?.remote) return null
  return entry.result.principal === principal ? entry : null
}
