...
| D28 | **Model selection (`--model name`, Phase 3) is fully protocol-driven, never hardcoded per backend** (2026-09-16) — ACP v1's `session/new` response can advertise `configOptions`, and a `category: "model"` entry lists real, agent-chosen choices (id/name pairs) discoverable at runtime. `core/acp-sessions.js`'s `applyModelSelection` matches the requested name against whichever option a backend tags `"model"` and sends `session/set_config_option` with the matched value — no backend-specific model-name tables to maintain or go stale. A backend that advertises no `"model"` option, or a name that doesn't match any choice, fails the session start clearly (listing available choices) rather than silently using the wrong model | `core/acp-sessions.js`'s `applyModelSelection` |
| D29 | **A live ACP session survives a listener restart (Phase 4, 2026-09-16), via `session/resume` first, `session/load` second, based on whichever capability the backend actually advertises at `initialize` — never assumed.** The ACP `sessionId` is persisted in `agent_sessions.metadata` (D25) at session start specifically so this is possible. The public `@agentclientprotocol/sdk` only builds its `ActiveSession` update-queue helper for `session/new`; resume/load have no equivalent public builder (verified from the SDK's own source, not assumed), so `core/acp-client.js` adds one hand-rolled piece: a global `session/update` notification tap, confirmed safe by reading `SessionUpdateRouter.handleMessage` — it always returns `Handled.no()`, i.e. taps without consuming, so it cannot starve `ActiveSession`'s own internal routing for every other (non-resumed) session. `routeThreadReply` attempts resume lazily, only when a thread reply arrives with no in-memory session; there is no eager resume-all-at-startup. Explicit `/agent-session close` now persists `status:'closed'` — without it, a closed session would look resumable forever | `core/acp-client.js`, `core/acp-sessions.js`'s `tryResumeSession` |
| D30 | **Ending a session must be gated, not left open to anyone in the channel** (2026-09-16, review fix) — `closeAgentSession` initially checked nothing at all. Also added: typing `stop`/`exit` as a plain reply in the session's thread closes it (`listen/socket.js`'s `SESSION_STOP_WORDS`), no slash command required — and closing now works even when the session was lost to a restart and never resumed (DB-only path, no in-memory entry needed), since ending a session is pure local bookkeeping, not something that requires talking to the backend at all. The specific gate this landed with is D31, below — an initial "same allowlist as starting" version was superseded same-day once it became clear start-rights and stop-rights are different trust levels | `core/acp-sessions.js`'s `closeAgentSession` |
| D31 | **Being allowed to start your OWN sessions does not make you allowed to stop someone else's** (2026-09-16, on request) — `isAllowedToCloseSession` is a separate check from `isAllowedToStartSession`: only the owner, the session's own starter (`entry.startedBy` in-memory, or `persisted.metadata.requestedBy` for a not-yet-resumed session), or someone in the new, distinct `agentSessions.allowedControllers` list may close a given session. Delegating "start/stop on my behalf, including other people's" is a materially bigger trust grant than "can start their own" and gets its own config key rather than overloading `allowedUsers` | `core/acp-sessions.js`'s `isAllowedToCloseSession` |
| D32 | **Pre-publish review fixes (2026-09-16), found live and by direct code inspection (OpenCode's own review tooling crashed locally on an unrelated skill-loading bug, unfixable from here — reviewed by hand instead):** (1) `core/acp-client.js` never passed `.env`-loaded credentials to the spawned backend process — it only inherited the listener's own `process.env`, so Bedrock/API-key config in `.env` was silently ignored and the backend fell back to its own (broken) default auth; confirmed live via a real "OAuth session expired" failure. (2) `startAgentSession`/`tryResumeSession` had no error handling around session creation/resume at all — any throw (spawn failure, `session/new` rejection, a DB error) propagated uncaught to Bolt's `app.error`, leaving the Slack message stuck on "starting…"/silently dropping the triggering reply, with zero user-visible feedback. (3) `core/acp-fs.js`'s repo-boundary check (D23) only did string-level `path.resolve()` normalization, never touching the filesystem — a symlink INSIDE the repo pointing outside it would pass the check while actually reading/writing elsewhere; now also `realpathSync`s the deepest existing ancestor and checks THAT against the repo root's realpath. (4) Two rapid messages in the same thread could race into `tryResumeSession` simultaneously, each registering under the same `acpSessionId` and clobbering the other's handlers; `routeThreadReply` is now serialized per `{channel, threadTs}` via `withKeyLock`, without blocking unrelated threads | `core/acp-client.js`, `core/acp-sessions.js`, `core/acp-fs.js` |
| D33 | **A DM directly to the bot can start a session too, not just @mention/slash-command/shortcut** (2026-09-16, on request) — @mention doesn't apply in a DM (there's nothing to @-mention when already talking to the app directly), so `listen/socket.js`'s `app.message` handler checks the SAME `agentSessions.mentionKeyword` prefix for `channel_type === 'im'` messages, with no channel involved at all — the session lives entirely in the DM thread | `listen/socket.js` |
| D34 | **Claude Code hooks, not ACP, are the enforcement point for sessions the bridge did not spawn** (2026-09-19, on request) — ACP installs its `onPermissionRequest` callback at spawn, when the bridge becomes the agent's parent and owns its stdio, so a session the user started in their own terminal can never be adopted mid-way; that would require the harness to expose an ACP control socket on a live session. Hooks are invoked by the harness itself, run as short-lived processes from an arbitrary cwd, and work on any session including ones the bridge never spawned. Away mode is therefore a hook feature gated by its own flag file (`<TSB_HOME>/.away`), and ACP's existing in-process path is left unchanged | `core/hooks.js`, `core/away.js`, `cli/hook.js` |
| D35 | **Away mode fails OPEN, and fail-open for `PreToolUse` is "no output at all", not a decision value** (2026-09-19, doc-verified) — on a timeout, a stopped listener, a Slack error or a db error, the hook emits no JSON and exits 0, which the harness documents as "Exit 0 with no output means the hook has no decision to report, so the tool call continues through the normal permission flow. The hook can deny the call, but staying silent doesn't approve it." An earlier draft of this feature specced `permissionDecision: "ignore"`; that value does not exist in the contract. Failing open means an unanswered request degrades to the ordinary local prompt — never a silent approval, never a false denial | `core/hooks.js`'s `preToolUse`, `cli/hook.js` |
| D36 | **`Stop` blocks via exit 2 + stderr, with our own continuation cap rather than `stop_hook_active`** (2026-09-19, doc-verified) — the documented mechanism is the exit-code table's "Stop \| Yes \| Prevents Claude from stopping, continues the conversation"; a JSON `decision:"block"` body could not be confirmed on the page, and `stop_hook_active` could not be confirmed as an input field, so the loop guard is ours rather than the harness's: a per-session counter at `<TSB_HOME>/hooks/stop-<session_id>.json` capped at `maxContinuations` (default 3), written temp-then-rename so the worst case is a lost increment rather than a doubled block. Malformed or missing stdin on a `stop` hook exits 0 — a bug here must never wedge a session | `core/hooks.js`'s `stop`, `cli/hook.js` |
| D37 | **WAL + `busy_timeout` are a correctness prerequisite for N concurrent asks** (2026-09-19, review fix) — away mode is the first feature to create many concurrent short-lived reader processes (parallel tool calls, parallel subagents), each polling the shared `asks` table at 1 Hz while the listener writes to it. The db previously ran in sqlite's default rollback-journal mode with no `busy_timeout`, so a reader colliding with a writer's exclusive lock got `SQLITE_BUSY` immediately, and the `getAsk` read inside `waitForAnswer`'s loop was uncaught — a busy collision rejected the `ask()` promise and killed the caller instead of returning a result. `busy_timeout` is set BEFORE `journal_mode=WAL`, since the mode switch itself can block on a competing lock | `core/db.js`'s `getDb`, `core/ask.js`'s `waitForAnswer` |
| D38 | **Autocompact can be announced, not gated** (2026-09-19, doc-verified after an incorrect earlier claim) — the request was "always ask permission before autocompact". No blocking mechanism for `PreCompact` exists: `compactionDecision` appears nowhere in the harness docs, and exit 2 does not prevent compaction either. An earlier draft of this plan asserted a `compactionDecision: allow\|deny` field on the strength of a bad doc summary; it is not real. `preCompact` therefore matches `trigger === 'auto'` and sends an informational DM so the user knows it happened, and the limitation is documented rather than papered over | `core/hooks.js`'s `preCompact` |
