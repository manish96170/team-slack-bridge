...
| D30 | **Ending a session is gated by the same D24 allowlist as starting one** (2026-09-16, review fix) — `closeAgentSession` initially checked nothing, meaning anyone in the channel could end a session they didn't start. Fixed to require `isAllowedToStartSession`. Also added: typing `stop`/`exit` as a plain reply in the session's thread closes it (`listen/socket.js`'s `SESSION_STOP_WORDS`), no slash command required — and closing now works even when the session was lost to a restart and never resumed (DB-only path, no in-memory entry needed), since ending a session is pure local bookkeeping, not something that requires talking to the backend at all | `core/acp-sessions.js`'s `closeAgentSession` |
