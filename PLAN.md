...
```
core/            slack.js (one HTTP call helper), post.js, dm.js, react.js, query.js,
                 classify.js (pure), identity.js, ledger.js (idempotency), errors.js
listen/          socket.js (Socket Mode client), envelope.js
cli/             post, dm, reply, react, query, listen, setup, doctor, get-user-token
mcp/             stdio.js, http.js          (transport bindings — no behaviour)
                 tools.local.js             (full set)
                 tools.remote.js            (channel-post only; imports NO user-token module — §11.2)
...
3. **[DONE 2026-09-09] MCP server** over the same core. *Gate met: hand-verified over stdio —
   `initialize` → `tools/list` (12 tools) → `tools/call` on `slack_post` with `dryRun: true` — round-tripped
   correctly; captured stdout contained only JSON-RPC frames, the startup banner went to stderr.*
   `mcp/{schema,tools.local,server}.js`, stdio transport, hand-rolled JSON-RPC (no dependency — see the note
   in `mcp/server.js` on why, and revisit if the remote/HTTP transport needs more of the spec).
   `mcp/tools.remote.js` **not built yet** — that's step 10, after the open questions in §9 and the listener
   steps 6–9, per this section's own ordering.
...
8. **Query/read methods** — the functions and CLI/MCP wiring already exist (`core/query.js`,
   ...
   path IS verified (`core/query.js` returns `{ ok:false, error:'search-requires-user-token' }` before ever
   calling Slack).*
...
10–13. **Not started.** Registry split (`tools.remote.js`), remote HTTP transport, remote hardening, remote
   extras — all correctly still blocked behind steps 6–9 per this section's own ordering; nothing here changes
   that.
...
| `node:sqlite` (Node built-in, not an npm package) | 2026-09-09 | D10/D11 — ledger concurrency once the listener can post alongside a CLI/MCP/dashboard call |

**10–13 remain as originally scoped** (registry split with the equality test written first, the HTTP transport
binding constructible only with the remote registry, remote hardening, the safe-on-remote extras) — see §11.2,
§11.3, §11.6 for the gates. Nothing about D8–D13 changes any of that; they were all local-profile decisions.
...
- `mcp/tools.remote.js` — channel post, thread reply, react, update/delete. **It does not import**
  `postAsUser`, `dm`, `search`, or the listener. There is no code path from the remote transport to them, so
  they are unreachable by **omission**, not by a check that could be inverted.
...
set — not a subset check, an equality check — and that importing `tools.remote.js` does not transitively pull in
the user-token module. A new tool added to the local set must therefore be a deliberate decision to expose or
withhold, and a test failure is the prompt to make it. (The dashboard's capability-coverage case works this way,
and it immediately caught two entries nobody had classified.)
...
1. **Who may call it.** Still required — otherwise anyone with the URL posts as your company's automation. This
   is MCP's OAuth 2.1 resource-server story (§10.2, identity #1). Identity #2 — the Slack-person binding —
   disappears entirely, which is the whole point of §11.1.
2. **A channel allow-list.** An authenticated caller must not be able to post into *every* channel the bot was
   ever invited to. Config lists the permitted channels; anything else is refused by name.
3. **Attribution on every message.** The bot posts, but *someone* asked it to. Carry the calling principal in
   the message (a footer, or Block Kit context) so a channel reader can tell which caller produced it. Posting
   anonymously as shared automation is how a shared bot becomes deniable.
4. **A per-caller quota.** Slack rate limits per app, so one noisy caller starves everyone on that instance.
   A simple per-principal budget plus the §4.1 rate-limit handling is enough.

Plus the audit log that already falls out of §4.1's idempotency ledger: principal, tool, channel, result.
